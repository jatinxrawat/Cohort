...
const OptionWheel = forwardRef(({
  ...
  const runFrame = useCallback(now => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      // Fast hardware-accelerated 3D transform without calc() or dynamic blur CSS overhead
      el.style.transform = `translate3d(${x.toFixed(1)}px, ${(y - cfg.rowH * 0.5).toFixed(1)}px, 0px) rotate3d(0, 0, 1, ${rot.toFixed(2)}deg)`;
  ...
  useImperativeHandle(ref, () => ({
    stepNext: () => applyTarget(Math.round(targetRef.current) + 1, true),
    stepPrev: () => applyTarget(Math.round(targetRef.current) - 1, true)
  }), [applyTarget]);

  // Wheel / touchpad scrolling locked to strict single-step navigation
  useEffect(() => {
  ...
  const handlePointerMove = useCallback(
    e => {
      ...
      if (dragMovedRef.current) {
        // Dampen drag velocity for weighted, butter-smooth dragging
        const dragFactor = 0.5;
        applyTarget(drag.start - (dy / cfgRef.current.rowH) * dragFactor, false);
      }
    },
    [applyTarget]
  );
