...
export default function HomeRightPanel() {
  ...
  return (
    <aside className="hidden xl:flex flex-col w-[360px] flex-shrink-0 sticky top-[72px] self-start max-h-[calc(100vh-88px)] overflow-y-auto overflow-x-hidden ml-4 pr-0.5 pb-8 gap-4 scrollbar-none" style={{ scrollbarWidth: 'none' }}>
      ...
      <div className="rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800/50 p-4 shadow-sm">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-black text-[13px] tracking-tight text-neutral-900 dark:text-neutral-100 leading-tight">
                Cohort{' '}
        ...
        <Link
          to="/uncut"
          className="w-full flex items-center justify-between px-3.5 py-2 rounded-xl bg-neutral-50 dark:bg-neutral-800/50 hover:bg-pink-500/8 dark:hover:bg-pink-500/10 text-neutral-500 dark:text-neutral-400 hover:text-pink-600 dark:hover:text-pink-400 transition-all duration-200 group"
        >
      ...
      {/* Featured Stories — scrollable list */}
      <div className="rounded-2xl bg-white dark:bg-neutral-900 border border-neutral-100 dark:border-neutral-800/50 shadow-sm overflow-hidden">
        {/* Sticky header inside card */}
        <div className="flex items-center justify-between px-4 pt-4 pb-3 bg-white dark:bg-neutral-900 sticky top-0 z-10">
          <div className="flex items-center gap-2">
            <Sparkles className="w-3.5 h-3.5 text-purple-500" />
            <h2 className="font-bold text-[13px] text-neutral-800 dark:text-neutral-200 tracking-tight">
              Featured Reads
            </h2>
          </div>
          <Link
            to="/uncut"
            className="text-[10px] font-semibold text-pink-500 dark:text-pink-400 hover:text-pink-600 dark:hover:text-pink-300 transition-colors"
          >
            See all
          </Link>
        </div>
          ...
          <div className="space-y-0.5">
            {FEATURED_ARTICLES.map((article) => (
              <Link
                key={article.id}
                to={`/uncut/${article.id}`}
                className="group flex gap-3 p-3 rounded-xl hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition-all duration-200 cursor-pointer"
              >
                {/* Gradient accent bar */}
        ...
        {/* Fade hint at bottom */}
        <div className="h-5 bg-gradient-to-t from-white dark:from-neutral-900 to-transparent -mt-5 relative z-10 pointer-events-none" />
      </div>
      ...
      {/* Footer links */}
      <div className="px-1">
        <p className="text-[9px] text-neutral-300 dark:text-neutral-700 leading-relaxed">
          <Link to="/about" className="hover:text-neutral-500 dark:hover:text-neutral-500 transition-colors">About</Link>
          {' · '}
          <Link to="/privacy" className="hover:text-neutral-500 dark:hover:text-neutral-500 transition-colors">Privacy</Link>
          {' · '}
          <Link to="/terms" className="hover:text-neutral-500 dark:hover:text-neutral-500 transition-colors">Terms</Link>
          {' · '}
          <Link to="/contact" className="hover:text-neutral-500 dark:hover:text-neutral-500 transition-colors">Contact</Link>
        </p>
        <p className="text-[9px] text-neutral-300 dark:text-neutral-700 mt-0.5">© 2026 Cohort</p>
      </div>
    </aside>
  );
}
