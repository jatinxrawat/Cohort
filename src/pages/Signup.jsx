...
export default function Signup() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrors({ email: err.message || 'Failed to send verification code. Please try again.' });
  ...
  const handleVerifyAndSignup = async (e) => {
    ...
    try {
      // Clear any existing stale Firebase session first to prevent requires-recent-login session conflicts
      ...
      if (!otpSnap.exists()) {
        setErrors({ otp: 'Verification code not found. Please request a new one.' });
      ...
      if (otpData.otp !== otpInput.trim()) {
        setErrors({ otp: 'Incorrect verification code. Please check and try again.' });
      ...
      if (timeDiff > 10 * 60 * 1000) { // 10 minutes expiry
        setErrors({ otp: 'Verification code expired. Please request a new one.' });
      ...
      localStorage.setItem('cohort_temp_signup_password', tempPassword);
      ...
      setErrors({ otp: err.message || 'Signup failed. Please try again.' });
  ...
  return (
    <div className="relative min-h-screen w-full bg-[#08080C] text-neutral-100 flex flex-col justify-start overflow-x-hidden select-none pb-0 lg:pb-24">
      <SEO 
      ...
      <main className="relative z-20 max-w-7xl w-full mx-auto px-6 sm:px-12 py-6 lg:pt-2 lg:pb-20 flex flex-col justify-center items-center lg:grid lg:grid-cols-12 lg:items-center gap-6 lg:gap-12 flex-1 overflow-y-auto">
        
        {/* LEFT COLUMN: TYPOGRAPHY + AUTH FORM */}
        <div className="lg:col-span-6 space-y-3.5 sm:space-y-4 text-center lg:text-left max-w-xl mx-auto w-full">
          ...
          {/* Social Auth Buttons */}
          <div className="max-w-sm space-y-2 pt-1 mx-auto lg:mx-0">
            {/* Social Auth Buttons (Hidden in Step 2 verification) */}
                  ...
                  <div>
                    <input
                      ...
                      className={`w-full bg-black border ${
                        errors.otp ? 'border-rose-500' : 'border-neutral-800 focus:border-purple-500'
                      } rounded-xl px-4 py-2.5 text-xs sm:text-sm text-center font-mono font-bold tracking-widest text-white placeholder-neutral-500 focus:outline-none focus:ring-1 focus:ring-purple-500/50 transition-all`}
                      required
                    />
                    {errors.otp && (
                      <p className="text-xs text-rose-400 mt-1 font-medium text-center">{errors.otp}</p>
                    )}
                  </div>
