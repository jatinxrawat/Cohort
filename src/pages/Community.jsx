...
export default function Community() {
  ...
  const handleCreateFeedPost = async () => {
    ...
    const senderName = user?.name || user?.email?.split('@')[0] || 'Student';
    const postData = { author: { name: senderName, avatar: user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(user?.email || senderName)}`, role: user?.college || 'Student' }, content: newPostText.trim(), timestamp: new Date(), likes: 0, comments: 0, shares: 0, liked: false, saved: false };
    try { const docRef = await addDoc(collection(db, 'community-feed'), postData); setFeedPosts(prev => [{ id: docRef.id, docId: docRef.id, ...postData }, ...prev]); setNewPostText(''); showSuccess('Announcement shared!'); } catch (e) { console.error(e); }
  };
                ...
                <div className="flex-1 overflow-y-auto px-lg py-md space-y-lg scrollbar-thin">
                  <Card className="mb-lg">
                    <div className="flex gap-md mb-lg">
                      <img src={user?.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${encodeURIComponent(user?.email || 'user')}`} alt="You" className="w-10 h-10 rounded-full" />
                      <div className="flex-1"><textarea placeholder="Share an announcement..." value={newPostText} onChange={(e) => setNewPostText(e.target.value)} rows={3} className="w-full bg-neutral-50/50 dark:bg-neutral-900/60 border border-neutral-200 dark:border-neutral-800 rounded-xl px-lg py-md text-sm outline-none resize-none focus:ring-2 focus:ring-primary-500" /></div>
                    </div>
                    <div className="flex justify-end pt-md border-t border-neutral-100 dark:border-neutral-800">
                      <Button variant="primary" size="sm" disabled={!newPostText.trim()} onClick={handleCreateFeedPost}>Share Announcement</Button>
                    </div>
                  </Card>
                  {feedPosts.length > 0 ? <div className="max-w-2xl mx-auto space-y-lg">{feedPosts.map(post => <PostCard key={post.id} post={post} onLike={handleLikePost} onSave={handleSavePost} />)}</div> : <Card className="text-center py-5xl max-w-2xl mx-auto"><AlertCircle className="w-10 h-10 text-neutral-300 dark:text-neutral-700 mx-auto mb-lg" /><h3 className="font-bold mb-xs">No Announcements Yet</h3><p className="text-sm text-neutral-500">Be the first to post!</p></Card>}
                </div>
              )}
